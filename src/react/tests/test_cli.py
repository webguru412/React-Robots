...
class TestModel(unittest.TestCase):
    def test_parse1(self):
        cmd, name, params, to = cli.parse("trigger Register()")
    ...
    def test_parse3(self):
        cmd, name, params, to = cli.parse("trigger Register(name='aleks')")
        self.assertEqual("trigger", cmd)
        self.assertEqual("Register", name)
        self.assertEqual({'name': 'aleks'}, params)

    def test_parse4(self):
        cmd, name, params, to = cli.parse("trigger Register(name='aleks', x = 1)")
        self.assertEqual("trigger", cmd)
        self.assertEqual("Register", name)
        self.assertEqual({'name': 'aleks', 'x': 1}, params)

    def test_parse5(self):
        cmd, name, params, to = cli.parse("trigger Register(name='aleks', x = 1) to @32")
        self.assertEqual("trigger", cmd)
        self.assertEqual("Register", name)
        self.assertEqual({'name': 'aleks', 'x': 1}, params)
        self.assertEqual(32, to)
